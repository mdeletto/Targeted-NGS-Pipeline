...
def apply_summary_filters(wb,sheetname,indices,transcript_id_row_dict,colocated_id_row_dict):
    ...
    for row in summary_sheet.rows:
        ...
        if row_counter==1: # pass for header
            ...
            if not re.search("HYPERLINK", str(pos.value)):
                ...
                match = re.search("""=HYPERLINK.+?(\".+?\").+?(\".+?\")""", pos.value)
                displayed_cell_value = match.group(2).strip('"') 
                pos_as_int = int(displayed_cell_value)
...
def main():
    ...
    try:
        wb = openpyxl.load_workbook(opts.input, data_only=False)
