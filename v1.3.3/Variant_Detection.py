...
def main():
    ...
    parser.add_option('--filter_common_germline', help="""Filter common snps (maf > 0.001) from germline output""", dest='filter_common_germline', action='store_true',default=False)
    ...
    group.add_option("--ionreporter_version", action="store", help="Please indicate IR version number (ex. 4.0, 4.4)", dest="ionreporter_version", default=5.6)
    ...
    VEP_EXE = "/home/michael/bin/ensembl-vep/vep"
    ...
    CNV_PLOT_EXE = '/home/michael/YNHH/Code/Github-mdeletto/Targeted-NGS-Pipeline/next-release/CNV_plot_ionreporter.R'
    PP_PARSER_EXE = '/home/michael/YNHH/Code/Github-mdeletto/Targeted-NGS-Pipeline/next-release/pipeline-parser.py' # % vers
    FILTER_VARIANTS_EXE = '/home/michael/YNHH/Code/Github-mdeletto/Targeted-NGS-Pipeline/next-release/filter_variants.py' # % vers
    ...
    MUTECT2_PON_OCPv3 = "/home/michael/YNHH/Reference_Files/tool-reference-files/MuTect2/PON/OCPv3/OCPv3.mutect2.PON.vcf"
    
    # POPULATION NORMALS
    
    POPULATION_NORMAL_BAM_OCP = '/home/michael/YNHH/Reference_Files/Population_Normal/PopulationNormal_OCP_v2.bam'
    POPULATION_NORMAL_BAM_OCPv3 = '/home/michael/YNHH/Reference_Files/Population_Normal/OCP/v3/PopulationNormal_OCP_v3.downsampled.bam'
    ...
    if opts.url is False:
        ...
        if re.search("Population", opts.normal):
            ...
            elif opts.regions == "OCP" or opts.regions == "OCA" or opts.regions == "OCPv2" or opts.regions == "OCAv2": 
                opts.normal = POPULATION_NORMAL_BAM_OCP
            elif opts.regions == "OCPv3" or opts.regions == "OCAv3":
                opts.normal = POPULATION_NORMAL_BAM_OCPv3
    ...
    master_IR_dict = construct_master_IR_dict(opts)
    print master_IR_dict
    if opts.ionreporter_skip_all is False:
        
        for analysis_type in master_IR_dict:

            
            if master_IR_dict[analysis_type]['analysis_name']:
                print "Remote download of %s VCF from IonReporter initiated.  Please hold..." % str.upper(analysis_type)
                master_IR_dict[analysis_type]['url_encoded_analysis_name'] = urllib.quote_plus(master_IR_dict[analysis_type]['analysis_name'])
                try:
                    master_IR_dict = IR_locate_variant_zip(master_IR_dict, analysis_type)
                except Exception, e:
                    print str(e)
                    print "FAILED TO LOCATE %s VCF from IonReporter." % str.upper(analysis_type)
                try:
                    master_IR_dict = IR_download_variant_zip(VCFLIB_DIR, opts, master_IR_dict, analysis_type)
                    retries = 1
                    while master_IR_dict[analysis_type]['unfiltered_vcf'] not in os.listdir(os.getcwd()):
                        if retries <= 5:
                            print "WARNING: %s not found.  Waiting 2 seconds and trying again (current attempt: %s)" % (master_IR_dict[analysis_type]['unfiltered_vcf'], str(retries))
                            time.sleep(2)
                            retries += 1
                        else:
                            print "FAILED TO DOWNLOAD %s VCF from IonReporter." % str.upper(analysis_type)
                            sys.exit(1)
                except Exception, e:
                    print str(e)
                    print "FAILED TO DOWNLOAD %s VCF from IonReporter." % str.upper(analysis_type)
            ...
            if analysis_type == "somatic":
                if opts.ionreporter_somatic_url_bool is True:
                    ionreporter_somatic_unfiltered_vcf = master_IR_dict['somatic']['unfiltered_vcf']
                else:
                    ionreporter_somatic_unfiltered_vcf = opts.ionreporter_somatic_vcf
                
                master_IR_dict['somatic']['unfiltered_vcf'] = IR4_4_VCF_fix(ionreporter_somatic_unfiltered_vcf, opts.base_output)
                edit_IR_tsv_file(opts.ionreporter_version, master_IR_dict['somatic']['additional_tsv'], opts.base_output)
...
# 
#                 link_and_barcodes_list = IR_locate_variant_zip(url_encoded_ionreporter_somatic_analysis_name, opts.ionreporter_somatic_id)
#                 variant_link, sample_barcode, control_barcode = (i for i in link_and_barcodes_list)
#                 somatic_sample_dict = {"tumor": sample_barcode,
#                                         "normal": control_barcode}
# 
#                 # Download analysis and process files
#                 ionreporter_somatic_vcf_and_tsv = IR_download_somatic_variant_zip(opts.base_output, variant_link, "somatic", somatic_sample_dict)
...
#                 rename_fusion_vcf(opts.ionreporter_fusion_vcf,opts.base_output) # Rename fusions.vcf file as (basename).ionreporter.fusions.vcf
        ...
        if os.path.isfile('%s.ionreporter.fusions.vcf' % opts.base_output):
            ...
            if opts.ionreporter_fusion_vcf is True:
                rename_fusion_vcf(opts.ionreporter_fusion_vcf,opts.base_output)
            else:
                # If no fusions.vcf file, then create a blank for Downstream upload
                subprocess.call("cat /home/michael/YNHH/Reference_Files/IR/fusions.v4.4.vcf > %s.ionreporter.fusions.vcf" % opts.base_output,shell=True)
                #open('%s.ionreporter.fusions.vcf' % opts.base_output, 'a').close()
...
#         subprocess.call("rm %s" % opts.ionreporter_somatic_vcf, shell=True)
    ...
    if opts.ionreporter_only is False:
        if opts.regions == "CCP":
            ...
            mutect2_unfiltered_vcf = muTect2_caller_command(GATK_LATEST_EXE,REGIONS_FILE,MUTECT2_PON,dbsnp_vcf,cosmic_vcf,REFERENCE_FASTA,opts.normal,opts.tumor,opts.base_output)
        elif opts.regions in ["OCP","OCA","OCPv2","OCAv2"]:
            MUTECT2_PON = MUTECT2_PON_OCP
            mutect2_unfiltered_vcf = muTect2_caller_command(GATK_LATEST_EXE,REGIONS_FILE,MUTECT2_PON,dbsnp_vcf,cosmic_vcf,REFERENCE_FASTA,opts.normal,opts.tumor,opts.base_output)
        elif opts.regions in ["OCPv3", "OCAv3"]:
            MUTECT2_PON = MUTECT2_PON_OCPv3
            mutect2_unfiltered_vcf = muTect2_caller_command(GATK_LATEST_EXE,REGIONS_FILE,MUTECT2_PON,dbsnp_vcf,cosmic_vcf,REFERENCE_FASTA,opts.normal,opts.tumor,opts.base_output)
...
#                                        )
    ...
    if opts.ionreporter_only is True:
        vcfs_and_basenames = {"ionreporter.somatic.unfiltered" : master_IR_dict['somatic']['unfiltered_vcf']} 
    ...
    if opts.ionreporter_germline_url_bool is True:
        vcfs_and_basenames.update({"ionreporter.germline.unfiltered" : master_IR_dict['germline']['unfiltered_vcf']})
    ...
    for description,vcf in vcfs_and_basenames.iteritems():
        if description == "ionreporter.somatic.unfiltered":
            master_IR_dict['somatic']['unfiltered_vcf'] = multibreak_vcf(VCFLIB_DIR, master_IR_dict['somatic']['unfiltered_vcf'], opts.base_output, description)
        elif description == "ionreporter.germline.unfiltered":
            master_IR_dict['germline']['unfiltered_vcf'] = multibreak_vcf(VCFLIB_DIR, master_IR_dict['germline']['unfiltered_vcf'], opts.base_output, description)
    ...
    if opts.ionreporter_skip_all is False:
        program_filter_vcf_list = [
                                   ...
                                   ["ionreporter.somatic",
                                    ...
                                    & (GEN[1].DP[*] >= 5)
                                    & !(ALT='<CNV>')""", 
                                    master_IR_dict['somatic']['unfiltered_vcf']],
                                       ...
                                       ["ionreporter.loh",
                                         """(HRUN[*] <= 6)
                                            ...
                                            & (GEN[0] != '.')
                                             ...
                                             & !(ALT='<CNV>')""", 
                                         master_IR_dict['germline']['unfiltered_vcf']]
                                       )
            ...
            if determine_num_variants_in_vcf(vcf) < 1:
                ...
                if program == "ionreporter.cnv":  
                    ...
                    if opts.filter_disabled is False:
                        ...
                        VEP_command_filtered(VEP_EXE, VEP_REF_FASTA, opts.base_output, program, filtered_vcf, DBNSFP, opts)
                    else:
                        VEP_command_unfiltered(VEP_EXE, VEP_REF_FASTA, opts.base_output, program, filtered_vcf, DBNSFP)

    #-------------------------------------------------------------------------------------------
    #--------------------------------CNV PLOTTING-----------------------------------------------
    #-------------------------------------------------------------------------------------------
    
    remote_dir_base_path = "/".join(master_IR_dict['somatic']['unfiltered_variants_link'].split("/")[:-1])
