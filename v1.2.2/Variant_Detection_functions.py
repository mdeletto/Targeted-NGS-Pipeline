...
def varscan_command(VarScan,mpileup,base_output, VCFLIB_DIR, MUTECT2_PON_VCF, REF_FASTA):
    ...
    try:
        ...
        subprocess.call("""%s/vcfintersect %s.indel.vcf \
                        -i %s \
                        -r %s \
                        -v > %s.indel.post-blacklist.vcf""" % (VCFLIB_DIR, base_output, MUTECT2_PON_VCF, REF_FASTA, base_output), shell=True)
        
        subprocess.call("java -jar %s processSomatic %s.indel.post-blacklist.vcf 2>> /tmp/error" % (VarScan,base_output),shell=True)
        subprocess.call("java -jar %s processSomatic %s.snp.vcf 2>> /tmp/error" % (VarScan,base_output),shell=True)

        # Combine snps and indels, and then sort 
        subprocess.call("vcf-concat %s.indel.post-blacklist.vcf %s.snp.vcf 2>> /tmp/error > %s.snp.indel.vcf" % (base_output,base_output,base_output),shell=True)
...
def IR_locate_variant_zip_by_id(ionreporter_id):
    """Find a variant zip from IonReporter by querying with analysis ID."""
    try:
        proc = subprocess.Popen(["""curl -k -H "Authorization:UmxyUXNPR3M1Q2RsbS9NYjBHQjBIaUxFTFA5RkJhRHBaMmlSSXZJTjBmUnNmQ0t1NkhOSUlrMStiNHFIQm16UjNKN2NYMzNOT2czcytqc2RveEhqK3BBSHhZNEhpNmRDVmtQaGRUZ1Z5ZXVXazJMTllQemIvV3A5c2NHOTNxRmY" "https://10.80.157.179/webservices_42/rest/api/analysis?format=json&id=%s" 2> /dev/null""" % ionreporter_id],shell=True,stdout=subprocess.PIPE)
        output, err = proc.communicate()
    except:
        print "Unable to communicate with server.  Check Authorization key, server address, and your network connectivity."
        sys.exit(1)
