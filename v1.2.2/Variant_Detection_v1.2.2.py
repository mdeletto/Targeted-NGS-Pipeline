...
parser = optparse.OptionParser(description=desc,version="%prog 1.2.2")
...
SAMTOOLS_EXE = "/home/michael/bin/samtools-1.3/samtools/samtools"
...
def main():
    header = "---STARTING TPL Targeted NGS Pipeline v1.2.2---"
    print "-" * len(header)
    print header
    print "-" * len(header)
    ...
    write_logfile(opts) # Write command line parameters t'/home/michael/YNHH/Reference_Files/tool-reference-files/MuTect2/PON/OCP/OCP.mutect2.PON.vcfo logfile 
    ...
    if opts.ionreporter_somatic_url_bool == True:
        ...
        try:
            variant_link = IR_locate_variant_zip_by_id(opts.ionreporter_somatic_id)
    ...
    if opts.ionreporter_fusion_url_bool == True:
        ...
        try:
            variant_link = IR_locate_variant_zip_by_id(opts.ionreporter_fusion_id) # Determine download link
...
#                                  strelka_vcf]
    ...
    if opts.tumor is None or opts.tumor == "None":
        ...
        if opts.regions=="CCP":
            MUTECT2_PON_VCF = MUTECT2_PON_CCP
        else:
            MUTECT2_PON_VCF = MUTECT2_PON_OCP
        ...
        varscan_command(VARSCAN_EXE,opts.base_output,varscan_base_output, VCFLIB_DIR, MUTECT2_PON_VCF, REF_FASTA)
