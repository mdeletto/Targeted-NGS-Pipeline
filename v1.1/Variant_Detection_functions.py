...
def samtools_command(tumor_bam,normal_bam,base_output,reference_fasta,regions_file):
    ...
    try:
        print "samtools mpileup -f %s -l %s %s %s 2>> /tmp/error > %s.mpileup" % (reference_fasta,regions_file,normal_bam,tumor_bam,base_output)
        subprocess.call("samtools mpileup -Q 7 -h 50 -o 10 -e 17 -m 4 -f %s -l %s %s %s 2>> /tmp/error > %s.mpileup" % (reference_fasta,regions_file,normal_bam,tumor_bam,base_output),shell=True) 
    except:
...
def VEP_command(VEP,REF_FASTA,base_output,program):
    ...
    print 'perl %s --quiet --cache --merged --offline --fasta %s -i %s.%s.vcf --everything --no_intergenic --filter no_stop_retained_variant --filter no_non_coding_transcript_exon_variant --filter no_intron_variant --filter no_synonymous_variant --json -o %s.%s.json -fork 16 2>> /tmp/error' % (VEP,REF_FASTA,base_output,program,base_output,program)
    try:
